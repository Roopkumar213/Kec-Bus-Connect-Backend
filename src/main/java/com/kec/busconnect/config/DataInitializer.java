...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        // Clean up any old admin record that was stored with bare "admin" email (migration fix)
        ...
        User adminUser = userRepository.findByEmail("admin@kec.ac.in").orElse(new User());
        adminUser.setEmail("admin@kec.ac.in");
        adminUser.setPasswordHash(passwordEncoder.encode("admin123"));
        adminUser.setRole(Role.ADMIN);
        adminUser.setActive(true);
        userRepository.save(adminUser);
        System.out.println("Default admin user created/updated (email: admin@kec.ac.in / password: admin123).");


        // Always ensure default driver user exists
        User driverUser = userRepository.findByEmail("driver@kec.ac.in").orElse(new User());
        ...
        System.out.println("Enforced single active bus KEC-07 and route 'Attikuppam → KEC (via MDR87)' (16 stops, 39.8 km).");
